...
- **rules/analyzers** apply the heuristics shipped in `graphqlmap/core.py`.
- **score** ranks by severity.
- **MCP server** (`graphqlmap mcp`) exposes `scan` for Cognis.Studio agents.
